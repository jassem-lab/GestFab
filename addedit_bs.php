...
<div class="wrapper">
  ...
  <div class="page-title-box">
	<div class="container-fluid">
		<div class="row">
			<div class="col-sm-12">
				<h4 class="page-title">Gestion des sorties de stock</h4>
  ...
  <script>
		function Supprimer(id,idd)
	  {
			...
			{
				document.location.href="page_js/supprimerdet_bs.php?IDD="+idd+"&ID="+id ;
			}			    
	  }	
  </script>
 <?php
	...
	if(isset($_GET['ID'])){
		...
		$date = date("Y");
		$max="01";
		$reqm="SELECT * from  erp_fab_compteur_bs  where date='".$date ."'" ;
		$querym=mysql_query($reqm);
		$numc=mysql_num_rows($querym);
			if($numc>0){
				$reqmax="SELECT max(compteur)+1 as MaxID from  erp_fab_compteur_bs  where date='".$date ."'";
...
if(isset($_POST['enregistrer_mail'])){	
	...
	if($id=="0")
		{		
			//Insertion entête
				$idbs="0";
				$reqMax="select max(id) as idbs from erp_fab_bs";
				$queryMax=mysql_query($reqMax);
				if(mysql_num_rows($queryMax)>0){
					while($enregMax=mysql_fetch_array($queryMax)){
						$idbs	=	$enregMax['idbs'] + 1;
					}
				} else{
						$idbs	=	"1";
				}
				$id=$idbs;
				$sql="INSERT INTO `erp_fab_bs`(`id`, `reference`, `fournisseur`, `date`, `dateheure`, `idutilisateur`, `observation`) values 
				('".$idbs."','".$reference."','".$fournisseur."' ,'".$date."','".$dateheure."' ,'".$idutilisateur."','".$remarque."')";
				$req=mysql_query($sql);
			
				//Mise à jour compteur référence
				$date1 = date("Y");
				$max="";
				$reqm="SELECT * from  erp_fab_compteur_bs  where date='".$date1 ."'" ;
				$querym=mysql_query($reqm);
				$numc=mysql_num_rows($querym);
					if($numc>0){
						$reqmax="SELECT max(compteur)+1 as MaxID from  erp_fab_compteur_bs  where date='".$date1 ."'";
						$querymax=mysql_query($reqmax);
						while($enregmax=mysql_fetch_array($querymax)){
							$max	=	$enregmax["MaxID"];
							if(strlen($max)==1){
					...
					$sql="INSERT INTO `erp_fab_compteur_bs`(`date`, `compteur`, `iddoc`) VALUES ('".$date1."' ,'".$max."','".$idbs."')";
					$req=mysql_query($sql);	
					
					//Insertion détails
					if($type==1){ //Commander des semi-finis
						$sql="INSERT INTO `erp_bc_det_bs`(`idbs`, `produit`, `prix_unitaire`, `quantite`, `prix_total`, `type`) VALUES";
						$sql=$sql."('".$idbs."' ,'".$produit."','".$prix_unitaire."','".$quantite."','".$prix_total."','".$type."') ";
						$req=mysql_query($sql);	
						...
						while($enregstock=mysql_fetch_array($querystock)){
							$stock = $enregstock['stock'] - $quantite;
						}	
						$sql="update erp_fab_produits set stock='".$stock."' where  id=".$produit;
						$req=mysql_query($sql);	
						
					} else{
						$sql="INSERT INTO `erp_bc_det_bs`(`idbs`, `mp`, `prix_unitaire`, `quantite`, `prix_total`, `type`) VALUES";
						$sql=$sql."('".$idbs."' ,'".$mp."','".$prix_unitaire."','".$quantite."','".$prix_total."','".$type."') ";	
						$req=mysql_query($sql);	
			...
			if($idd==0){
				...
				//MAJ détails
				$sql="update erp_bc_det_bs set prix_unitaire='".$prix_unitaire."', quantite='".$quantite."', prix_total='".$prix_total."'
				where id=".$idd;
				$req=mysql_query($sql);
				...
				if($type==1){
					...
					$querystock=mysql_query($reqstock);
					while($enregstock=mysql_fetch_array($querystock)){
						$stock = ($enregstock['stock']+$ancien_qte) - $quantite;
					}	
					$sql="update erp_fab_produits set stock='".$stock."' where  id=".$produit;
					$req=mysql_query($sql);						
				} else{
			...
			//Mise à jour montant total de BC
				...
				$sql="UPDATE erp_fab_bs set montant='".$total."',observation='".$remarque."' where id=".$id;
				$req=mysql_query($sql);	
		
		

		echo '<SCRIPT LANGUAGE="JavaScript">document.location.href="addedit_bs.php?ID='.$id.'&suc=1" </SCRIPT>';
	...
	$req="select * from erp_fab_bs where id=".$id;
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
...
<?php
	...
	if(isset($_GET['ID'])){
	 ...
	 $req="select * from erp_bc_det_bs where idbs=".$id." order by id desc ";
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
		$iddet				=	$enreg["id"] ;	
	...
	?>
										<tr>
											...
											<?php } ?>
											 ...
											 <td>
												<a href="addedit_bs.php?IDD=<?php echo $iddet; ?>&ID=<?php echo $id; ?>" class="btn btn-warning waves-effect waves-light">
...
<script>
	$("#type").on("change", function(){
		...
		if(type==""){
			...
			xmlhttp_listeproduitsf.open("POST","page_json/json_select_bs_produitsf.php",true);
			...
			xmlhttp_listeproduitmp.open("POST","page_json/json_select_bs_mps.php",true);

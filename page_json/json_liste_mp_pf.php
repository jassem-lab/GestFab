...
<script>
	...
	$(".btnModif1").on("click", function(){
		...
		if(confirm('Confirmez-vous la modification?')){
			...
			$.post("page_ajax/ajax_modifnomenclature_pf1.php", variable, function (data, status) {
				if (status == "success") {
						...
						xmlhttp_listemps.open("POST","page_json/json_liste_mp_pf.php",true);
						...
						//Refraiche la liste des MPs
							...
							xmlhttp_selectmps.open("POST","page_json/json_selectmp_pf.php",true);
	...
	$(".btnDelete").on("click", function(){
		...
		if(confirm('Confirmez-vous la suppression?')){
			var variable="id="+id;
			$.post("page_ajax/ajax_deletenomenclature_pf1.php", variable, function (data, status) {
				if (status == "success") {
						if (window.XMLHttpRequest)
						{
						  xmlhttp_listemps=new XMLHttpRequest();

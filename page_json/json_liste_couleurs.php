...
<table class="table mb-0">
	...
	<tbody>	
<?php
	$reqnom="select * from erp_fab_couleurs";
...
?>
			<tr>
				<td>
					<span id="span_moule<?php echo $enregnom['id']; ?>"> <?php echo $enregnom['couleur']; ?></span>
					<input style="display:none;width:;" id="moule<?php echo $enregnom['id']; ?>" class="form-control" value="<?php echo $enregnom['couleur']; ?>" >				
				</td>
				<td>
...
<script>
	...
	$(".btnModif1").on("click", function(){
		...
		if(confirm('Confirmez-vous la modification?')){
			var variable="couleur="+moule+"&des="+des+"&id="+id;
			$.post("page_ajax/ajax_edit_couleur.php", variable, function (data, status) {
				if (status == "success") {
						...
						xmlhttp_listemps.onreadystatechange=function(){
							...
							if(xmlhttp_listemps.status==200 && xmlhttp_listemps.readyState==4){
								...
								$('#listeCOL').html(xmlhttp_listemps.responseText);
							}	
						}
						xmlhttp_listemps.open("POST","page_json/json_liste_couleurs.php",true);
						...
						//Refraiche la liste des MPs
							...
							xmlhttp_selectmps.onreadystatechange=function(){
								...
								if(xmlhttp_selectmps.status==200 && xmlhttp_selectmps.readyState==4){
									...
									$('#listeCOL').html(xmlhttp_selectmps.responseText);
								}	
							}
							xmlhttp_selectmps.open("POST","page_json/json_liste_couleurs.php",true);
	...
	$(".btnDelete").on("click", function(){
		...
		if(confirm('Confirmez-vous la suppression?')){
			var variable="couleur="+moule+"&des="+des+"&id="+id;
			$.post("page_ajax/ajax_delete_couleur.php", variable, function (data, status) {
				if (status == "success") {
						if (window.XMLHttpRequest)
						{
						  xmlhttp_listemps=new XMLHttpRequest();

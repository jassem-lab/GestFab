...
<table class="table mb-0">
	...
	<tbody>	
<?php
	$reqnom="select * from erp_fab_jig";
...
?>
			<tr>
				<td>
					<span id="span_moule<?php echo $enregnom['id']; ?>"> <?php echo $enregnom['jig']; ?></span>
					<input style="display:none;width:;" id="moule<?php echo $enregnom['id']; ?>" class="form-control" value="<?php echo $enregnom['jig']; ?>" >				
				</td>
				<td>
...
<script>
	...
	$(".btnModif1").on("click", function(){
		...
		if(confirm('Confirmez-vous la modification?')){
			var variable="jig="+moule+"&des="+des+"&id="+id;
			$.post("page_ajax/ajax_edit_jig.php", variable, function (data, status) {
				if (status == "success") {
						...
						xmlhttp_listemps.onreadystatechange=function(){
							...
							if(xmlhttp_listemps.status==200 && xmlhttp_listemps.readyState==4){
								...
								$('#listeJIG').html(xmlhttp_listemps.responseText);
							}	
						}
						xmlhttp_listemps.open("POST","page_json/json_liste_jigs.php",true);
						...
						//Refraiche la liste des MPs
							...
							xmlhttp_selectmps.onreadystatechange=function(){
								...
								if(xmlhttp_selectmps.status==200 && xmlhttp_selectmps.readyState==4){
									...
									$('#listeJIG').html(xmlhttp_selectmps.responseText);
								}	
							}
							xmlhttp_selectmps.open("POST","page_json/json_liste_jigss.php",true);
	...
	$(".btnDelete").on("click", function(){
		...
		if(confirm('Confirmez-vous la suppression?')){
			var variable="jig="+moule+"&des="+des+"&id="+id;
			$.post("page_ajax/ajax_delete_jig.php", variable, function (data, status) {
				if (status == "success") {
						if (window.XMLHttpRequest)
						{
						  xmlhttp_listemps=new XMLHttpRequest();
						...
						//Refraiche la liste des MPs
							...
							xmlhttp_selectmps.open("POST","page_json/json_liste_jigs.php",true);
							xmlhttp_selectmps.setRequestHeader("Content-type","application/x-www-form-urlencoded");
							xmlhttp_selectmps.send("code="+moule);	
						//Fin refraiche liste des MPs	
				}

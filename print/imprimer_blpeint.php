...
function addClientAdresse( $clt,$adr,$tel ,$trim )
{
	...
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(33,8, "Telephone:", 0, 0, "C");
	$this->SetFont( "Arial", "", 8);
...
$req="select * from erp_bc_blfinis where id=".$id;	
...
while($enreg=mysql_fetch_array($query))
{
        ...
        $idbc_original      =   $enreg["idbc_original"] ; 
	...
	$pdf->fact_dev( "Reference", $reference );
	$pdf->Ln();$pdf->Ln();$pdf->Ln();
		
	$pdf->addDate( $date );
	$pdf->addPageNumber("1");
	if($client<>""){
		$pdf->addClientAdresse($client,$tel,$adresse,$date);
	}

    $ref_bc=0;
    $reqtrim="select * from erp_bc_blfinis where id=".$idbc_original;
    $querytrim=mysql_query($reqtrim);
    while($enregtrim=mysql_fetch_array($querytrim)){
        $ref_bc	=	$enregtrim['reference'];
    }	
...
$total=0;$tot_qte=0;
$prix_unitaire = 0 ; 
$pdf->SetWidths(array(50,30,20,20,20,30,30,30));
srand(microtime()*1000000);
$pdf->Ln();$pdf->Ln();
$des="";
$article ="" ; 
$pdf->RowHead(array(("Produit"),("couleur"),("Poids"),("Volume"),("Prix unitaire"),("Quantite"),("Prix total")));
$couleur = "" ;

        // FETCH BC PEINT DETAILS 

	$article="";
	$req="select * from erp_bc_detblfinis where idbl=".$id." order by id desc ";
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
		$iddet				=	$enreg["id"] ;	
        ...
        $couleurID          =   $enreg["couleur"] ; 
        $poids				=	0;
		$volume				=	0;
        $produit            =   $enreg["produit"] ; 
        //  FETCH COLORS

            $reqC = "select * from erp_bc_couleur where id=".$couleurID ; 
            $queryC = mysql_query($reqC) ; 
            while($enregC = mysql_fetch_array($queryC)){
                $couleur = $enregC["couleur"] ; 
            }
        // FETCH PRODUCT DETAILS 

		$reqfm="select * from erp_bc_produits where id=".$produit;
		$queryfm=mysql_query($reqfm);
		while($enregfm=mysql_fetch_array($queryfm)){
		...
		$pdf->Row(array(utf8_decode($article),($couleur),($poids),($volume),(number_format($prix_unitaire,'3','.','')),($qte),(number_format($enreg["prix_total"],'3','.',''))));		
...
$pdf->Ln();$pdf->Ln();
	...
	$pdf->RowHead(array(utf8_decode(''),(''),('TOTAL'),number_format($montant,'3','.','')));		

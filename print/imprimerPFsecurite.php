...
function addClientAdresse( $clt,$adr,$tel ,$trim )
{
	...
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(33,8, "T�l�phone:", 0, 0, "C");
	$this->SetFont( "Arial", "", 8);
...
//$pdf->Image('../assets/pied.png', 0, 276, 210 , 0, 'PNG');
	...
	$r2     = $r1 + 12;
	$pdf->SetXY( $r1, $y1+10);
	$pdf->MultiCell( 140, 4,(utf8_decode("List de stock des Produits Semi finis < stock de sécurité ")));		
	...
	$r2     = $r1 + 12;
	$pdf->SetXY( $r1, $y1+10);
    $reqPx = "SELECT SUM( `stock` ) AS stk, SUM( `prix` ) AS px  FROM `erp_fab_produits` where semi = 0 and stock < seuil";
	...
	$pdf->MultiCell( 140, 4,(utf8_decode("Prix total de stock : ".$stk * $px )));	

    $total_tt=0;
    $total=0;$tot_qte=0;
    $pdf->SetWidths(array(20,40,20,30,20,40,20));
    srand(microtime()*1000000);
    $pdf->Ln();$pdf->Ln();
    $des="";
    $pdf->RowHead(array((utf8_decode("Code")),(utf8_decode("Designation")),(utf8_decode("Provenance")),(utf8_decode("Prix")),(utf8_decode("Stock")),(utf8_decode("Valeur de Stock")),(utf8_decode("Seuil"))));


    $req="select * from erp_fab_produits where 1=1 and semi = 1 and stock < seuil order by code";
	...
	$type				=	"" ;	
	$i					=	0;
	$req="select * from erp_fab_produits where 1=1 and semi = 1 and stock < seuil";
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
		$id					=	$enreg["id"] ;	
		...
		$i++;
		$valeurStock        =   $enreg["prix"] * $enreg["stock"] ; 
		$provenance="";
        ...
        $pdf ->Row(array(utf8_decode($code) , $designation, $provenance,$prix,$enreg["stock"],$valeurStock,$enreg["seuil"]));
	}

$filename="Liste_stock_sf.pdf";

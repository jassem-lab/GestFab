...
$req="select * from erp_bc_br where id=".$id;	
...
while($enreg=mysql_fetch_array($query))
{
	...
	$pdf->addPageNumber("1");
	if($client<>""){
		$pdf->addClientAdresse($client,$tel,$adresse);
	}

$total_tt=0;
$total=0;$tot_qte=0;
$pdf->SetWidths(array(80,30,20,30,30,30));
srand(microtime()*1000000);
$pdf->Ln();$pdf->Ln();
$des="";
$pdf->RowHead(array(("Produit"),("Couleur"),("type de defaut"),("Quantite"),("note")));
	$article="";
	$req="select * from erp_bc_det_br where idbr=".$id." order by id desc ";
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
		$iddet				=	$enreg["id"] ;	
		...
		$reqC="select * from erp_bc_couleur where id=".$couleur; 
        $queryC=mysql_query($reqC) ; 
        while($enregC=mysql_fetch_array($queryC)){
            $couleurName = $enregC['couleur']; 
        }

		$pdf->SetFont('Arial','', 10); // Font Name, Font Style (eg. 'B' for Bold), Font Size
		$pdf->Row(array(($article),($couleurName),($typedefaut),($quantite),($note)));		
...
$pdf->Ln();$pdf->Ln();
	...
	// $pdf->RowHead(array(utf8_decode(''),(''),(''),('TOTAL'),number_format($prix,'3','.',''),''));		

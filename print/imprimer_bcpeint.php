...
$req="select * from erp_bc_bcpeint where id=".$id;	
...
while($enreg=mysql_fetch_array($query))
{
    ...
    $ref_bc=0;
    $reqtrim="select * from erp_bc_bc where id=".$idbc_original;
    $querytrim=mysql_query($reqtrim);
    while($enregtrim=mysql_fetch_array($querytrim)){
...
$couleur = "" ;
	...
	$article="";
	$req="select * from erp_bc_det_bcpeint where idbc=".$id." order by id desc ";
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
		$iddet				=	$enreg["id"] ;	

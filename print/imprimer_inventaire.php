...
//$pdf->Image('../assets/pied.png', 0, 276, 210 , 0, 'PNG');
	...
	$r2     = $r1 + 12;
	$pdf->SetXY( $r1, $y1+10);
	$pdf->MultiCell( 140, 4,(utf8_decode("Inventaire de stock")));		
    ...
    $pdf->Ln();$pdf->Ln();
    $des="";
    $pdf->RowHead(array((utf8_decode("Type")),(utf8_decode("Code ( Designation ) ")),(utf8_decode("Quantite"))));
	...
	if(isset($_GET['ID'])){
	 ...
	 $req="select * from erp_bc_det_inventaire where idinventaire=".$id." order by id desc ";
	...
	while($enreg=mysql_fetch_array($query))
	{
        ...
        if($enreg['type']==1){ 
            echo  $pdf ->Row(array(utf8_decode($type) , $sproduit, $qte));
            }else{ 
                echo  $pdf->Row(array(utf8_decode($type) , $mp , $qte ));
           } 
      
		
	}

$filename="Inventaire_de_stock.pdf";

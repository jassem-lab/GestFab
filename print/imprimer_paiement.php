...
$req="select * from erp_bc_paiementsf where id=".$id;	
...
while($enreg=mysql_fetch_array($query))
{
		...
		while($enregfm=mysql_fetch_array($queryfm)){
			$client2				=	$enregfm["raisonsocial"] ;
			$tel				=	$enregfm["telephone"] ;
			$adresse			=	$enregfm["adresse"] ;
		}
        $reqfn = "select * from erp_bc_factsf where id=".$idfacture ;
        $queryfn = mysql_query($reqfn); 
        while($enregfn=mysql_fetch_array($queryfn)){
            $referenceFacture = $enregfn['reference'] ;
        }
		if($type==0){
            $type_paie = 'Avance sur commande' ; 
        }else{
            $type_paie = 'Payement Facture' ; 
        }
	...
	$pdf->fact_dev( "R�f�rence", $reference );
	$pdf->Ln();$pdf->Ln();$pdf->Ln();
...
$pdf->Ln();$pdf->Ln();
$des="";
$pdf->RowHead(array(("Reference"),("Client"),("Facture"),("BC"),("Type"),("Montant")));
	$article="";
    $referenceBC="";
	 $req="select * from erp_bc_bc where id=".$refbc;
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
		$iddet				=	$enreg["id"] ;	
		$referenceBC		=	$enreg["reference"] ;		
        $pdf->SetFont('Arial','', 10); // Font Name, Font Style (eg. 'B' for Bold), Font Size
		$pdf->Row(array(utf8_decode($reference),($client2),($referenceFacture),($referenceBC),($type_paie),(number_format($montant,'3','.',''))));		

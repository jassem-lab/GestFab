...
include('../connexion/cn.php');
	...
	$sql = "delete from `erp_bc_det_be` WHERE id=".$idd;
	...
	$sql="UPDATE erp_fab_be set montant='".$total."' where id=".$id;
	$req=mysql_query($sql);		

	
	$req="select * from erp_bc_det_be where idbe=".$id;
	$query=mysql_query($req);
	if(mysql_num_rows($query)<1){
		$sql="delete from erp_fab_be where id=".$id;
		$requete = mysql_query($sql) ;
		
		$sql_compteur="delete from erp_fab_compteur_be where iddoc=".$id;

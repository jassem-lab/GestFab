...
<div class="wrapper">
 ...
 <?php
	...
	if(isset($_GET['ID'])){
		...
		$date = date("Y");
		$max="01";
		$reqm="SELECT * from  erp_fab_compteur_be  where date='".$date ."'" ;
		$querym=mysql_query($reqm);
		$numc=mysql_num_rows($querym);
			if($numc>0){
				$reqmax="SELECT max(compteur)+1 as MaxID from  erp_fab_compteur_be  where date='".$date ."'";
...
if(isset($_POST['enregistrer_mail'])){	
	...
	if($id=="0")
		{		
			//Insertion entête
				$idbe="0";
				$reqMax="select max(id) as idbe from erp_fab_be";
				$queryMax=mysql_query($reqMax);
				if(mysql_num_rows($queryMax)>0){
					while($enregMax=mysql_fetch_array($queryMax)){
						$idbe	=	$enregMax['idbe'] + 1;
					}
				} else{
						$idbe	=	"1";
				}
				$id=$idbe;
				$sql="INSERT INTO `erp_fab_be`(`id`, `reference`, `fournisseur`, `date`, `dateheure`, `idutilisateur`, `observation`) values 
				('".$idbe."','".$reference."','".$fournisseur."' ,'".$date."','".$dateheure."' ,'".$idutilisateur."','".$remarque."')";
				$req=mysql_query($sql);
			
				//Mise à jour compteur référence
				$date1 = date("Y");
				$max="";
				$reqm="SELECT * from  erp_fab_compteur_be  where date='".$date1 ."'" ;
				$querym=mysql_query($reqm);
				$numc=mysql_num_rows($querym);
					if($numc>0){
						$reqmax="SELECT max(compteur)+1 as MaxID from  erp_fab_compteur_be  where date='".$date1 ."'";
						$querymax=mysql_query($reqmax);
						while($enregmax=mysql_fetch_array($querymax)){
							$max	=	$enregmax["MaxID"];
							if(strlen($max)==1){
					...
					$sql="INSERT INTO `erp_fab_compteur_be`(`date`, `compteur`, `iddoc`) VALUES ('".$date1."' ,'".$max."','".$idbe."')";
					$req=mysql_query($sql);	
					
					//Insertion détails
					if($type==1){ //Commander des semi-finis
						$sql="INSERT INTO `erp_bc_det_be`(`idbe`, `produit`, `prix_unitaire`, `quantite`, `prix_total`, `type`) VALUES";
						$sql=$sql."('".$idbe."' ,'".$produit."','".$prix_unitaire."','".$quantite."','".$prix_total."','".$type."') ";
						$req=mysql_query($sql);	
						...
						$sql="INSERT INTO `erp_bc_det_be`(`idbe`, `mp`, `prix_unitaire`, `quantite`, `prix_total`, `type`) VALUES";
						$sql=$sql."('".$idbe."' ,'".$mp."','".$prix_unitaire."','".$quantite."','".$prix_total."','".$type."') ";	
						$req=mysql_query($sql);	
			...
			if($idd==0){
				...
				$ancien_qte	=	0;
				$req="select * from erp_bc_det_be where id=".$idd;
				$query=mysql_query($req);
				while($enreg=mysql_fetch_array($query)){
					$ancien_qte	=	$enreg['quantite'];
				}
				//MAJ détails
				$sql="update erp_bc_det_be set prix_unitaire='".$prix_unitaire."', quantite='".$quantite."', prix_total='".$prix_total."'
				where id=".$idd;
				$req=mysql_query($sql);
			...
			//Mise à jour montant total de BC
				$total="0";
				$reqTot="select sum(prix_total) as total from erp_bc_det_be where idbe=".$id;
				$queryTot=mysql_query($reqTot);
				while($enregTot=mysql_fetch_array($queryTot)){
					$total	=	$enregTot['total'];
				}
				
				$sql="UPDATE erp_fab_be set montant='".$total."',observation='".$remarque."' where id=".$id;
				$req=mysql_query($sql);	
	...
	$req="select * from erp_fab_be where id=".$id;
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
...
<?php
	...
	if(isset($_GET['ID'])){
	 ...
	 $req="select * from erp_bc_det_be where idbe=".$id." order by id desc ";
	$query=mysql_query($req);
	while($enreg=mysql_fetch_array($query))
	{
		$iddet				=	$enreg["id"] ;	
	...
	?>
										<tr>
											...
											<?php } ?>
											 ...
											 <td>
												<a href="addedit_be.php?IDD=<?php echo $iddet; ?>&ID=<?php echo $id; ?>" class="btn btn-warning waves-effect waves-light">
...
<script>
	$("#type").on("change", function(){
		...
		if(type==""){
			...
			xmlhttp_listeproduitsf.open("POST","page_json/json_select_be_produitsf.php",true);
			...
			xmlhttp_listeproduitmp.open("POST","page_json/json_select_be_mps.php",true);
